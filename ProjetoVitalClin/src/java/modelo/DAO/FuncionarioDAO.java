...
public class FuncionarioDAO {
    ...
    private static final String SELECT_ALL = "SELECT * FROM listafunc;";
...
"		f.contratoFK = ct.id and f.cargoFK = cg.id and u.tipoAcessoFK = t.id and f.situacao <> 2 and f.nome=?;";
...
"		ct.id as idcontrato, ct.descricao as descricaocontrato, cg.id as idcargo, cg.descricao as descricaocargo \n" +
"		from funcionario f, usuario u, tipoAcesso t, contrato ct, cargo cg where f.usuarioFK = u.id and \n" +
"		f.contratoFK = ct.id and f.cargoFK = cg.id and u.tipoAcessoFK = t.id and f.situacao <> 2 and cg.descricao =?;";
    private static final String BUSCAR_EMAIL = "SELECT u.id as idusuario, u.senha, u.tipoAcessoFK, f.nome, f.id as idfuncionario FROM usuario u, funcionario f WHERE f.usuarioFK = u.id and u.email=?;";
...
"		ct.id as idcontrato, ct.descricao as descricaocontrato, cg.id as idcargo, cg.descricao as descricaocargo \n" +
"		from funcionario f, usuario u, tipoAcesso t, contrato ct, cargo cg where f.usuarioFK = u.id and \n" +
"		f.contratoFK = ct.id and f.cargoFK = cg.id and u.tipoAcessoFK = t.id and f.situacao <> 2 and f.id=?;";
    private static final String UPDATE = "UPDATE funcionario SET nome=?, cpf=?, dtNascimento=?, sexo=?, logradouro=?, bairro=?, numero=?, cidade=?, uf=?, cep=?, telFixo=?, celular=? salario=? WHERE id=?;";
    ...
    public boolean alterar(Funcionario funcionario) throws SQLException {
        ...
        try {
            ...
            pstmt.setString(11, funcionario.getTelefoneFixo());
            pstmt.setString(12, funcionario.getCelular());
            pstmt.setDouble(13, funcionario.getSalario());
            pstmt.setInt(14, funcionario.getId());
            pstmt.execute();
            return true;
